...
namespace cs251
{
  ...
  void callbacks_t::keyboard_special_cb(int key, int x, int y)
  {
    ...
    case GLUT_KEY_LEFT:{
      b2Vec2 pos = slider->GetWorldCenter();
      slider->SetTransform( b2Vec2( pos.x-1.0f, 0.4f ), 0.0f );
	...
	if (state == GLUT_DOWN)
	  {
        ...
        //Toggle the effect when a body is clicked
        vector<pair<b2Vec2,bool> >::iterator it= static_bodies.begin();
        while(it!=static_bodies.end())
        {
          if((p-it->first).Length()<2.0){
            it->second = !it->second;
            break;
          }
          it++;
  ...
  void callbacks_t::restart_cb(int x)
  {
    delete test;
    entry = cs251::sim;
    test = entry->create_fcn();
