...
void base_sim_t::step(settings_t* settings)
{
  ...
  //force apply
  slider->SetLinearVelocity(b2Vec2(0.0f,0.0f));
  for (vector<b2Body*>::iterator itr = metals.begin(); itr!=metals.end(); itr++){
    b2Vec2 position = (*itr)->GetWorldCenter();
    if(position.y<0.71){

      cout<<"*****************GAME OVER****************"<<endl;
      cout<<"*             Your score was "<<metals.size()-1<<"           *"<<endl;
      cout<<"******************************************"<<endl;
       for (vector<b2Body*>::iterator itr1 = metals.begin(); itr1!=metals.end(); itr1++){
        //(*itr1)->SetTransform(b2Vec2(1000.0,0.0),0);
        m_world->DestroyBody(*itr1);
      }
      metals.clear();
      srand((int)time(0));
      int r = rand() % 390;
      metal m1((float)(r)/5.0-39.0,39.0,m_world);
      break;
    }
    if(position.y<2.0 && abs(position.x - slider->GetWorldCenter().x)<4.9){
      
      srand((int)time(0));
      int r = rand() % 390;
      int r1 = rand() % 390;
      (*itr)->SetLinearVelocity(b2Vec2(0.0f,0.0f));
      (*itr)->SetTransform(b2Vec2((float)(r1)/5.0-39.0,39.0),0);
      // (*itr)->SetActive(false);
      metal m1((float)(r)/5.0-39.0,39.0,m_world);//,*m2 = new metal((float)(r1)/5.0-39.0,39.0,m_world);
      //metals[1]->SetTransform(b2Vec2(10000.0,0.0),0);
      break;
    }
    if(position.y<15.0){
      break;
    }
    vector<pair<b2Vec2,bool> >::iterator it = static_bodies.begin();
    while(it!=static_bodies.end()){
      b2Vec2 dir = (it->first-position);
      dir.Normalize();
      float32 force =  60000.0/((position-it->first).LengthSquared());
      if(it->second){
        force *= -1.0;
      }
      dir*=force;
      //if(len>=1){
        (*itr)->ApplyForceToCenter(dir,true);
      //}
      it++;
    }
  }
